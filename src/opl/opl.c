...
// NDS port: no SDL. The only available OPL backend is the software
// emulator driven by opl_nds_driver (defined in src/nds/opl_nds.c).
#include "i_timer.h"
...
// NDS port: no SDL. OPL_Delay is used only during chip detection (before
// the driver timer is running), so we implement it as a simple busy-wait
// based on the millisecond clock exported by i_timer_nds.c.
...
void OPL_Delay(uint64_t us)
{
    ...
    start = I_GetTimeMS();
    // I_GetTimeMS() has millisecond resolution; round up to the next ms.
    end = start + (uint32_t)((us + 999) / 1000);

    while (I_GetTimeMS() < end)
    {
        // Busy-wait. This is only called during OPL init/detection, which
        // happens once at startup, so the brief stall is acceptable.
    }

    // Keep the emulated OPL status timers in sync with the wall clock so
    // that OPL_Detect() observes the expected status bits.
    opl_nds_time_advance(us);
}
